...
def quiver_acceleration(u12, u3, m1=1, m2=1):
    ...
    u2 = u12[3:5, :].reshape(2, -1, 1, 1) # (2,T,1,1)
...
def guide(goal, init, t0=0, tf=3, N=1000, m = [1, .09, .0001]):
    """
    Accepts a goal ndarray (3,) and an initial guess init ndarray(18,).
    Finds initial velocity conditions that will execute a slingshot that passes through this point.
    Note that this only works for the planar problem.
    Note that the guess init must be reasonable, i.e. for a given goal, you have to make a good guess with the ICs.
    Note that this is not perfect, and is not necessarily guaranteed to give a slingshot if the guess isn't good enough.
    """

    # Set t_eval for when we simulate the gravitational dynamics
    t_eval=np.linspace(t0, tf, N)

    # Pull out the planar velocity ICs for the third body
    v3 = init[-3:-1]

    # Define a function to minimize that will get the third body to pass through the point goal
    def to_minimize(v3):
        """Accepts planar velocity ICs for the third body,
        and adjusts them so that the third body passes through goal."""

        # Make a copy of the ICs and set the planar velocity ICs for the third body to be those passed in
        y0 = init
        y0[-3:-1] = v3

        # Run the simulation with the current velocity data
        sol = simulate_mechanics_general(y0, (t0, tf), t_eval=t_eval, m =m)

        # Find the index/time step of the solution that gets closest to passing through goal and return the norm of the difference between goal and the solution at that time step index
        index = np.argmin(np.linalg.norm(sol[6:9, :].T - goal, axis=1, ord=2))
        sol = sol[:, index]
        return np.linalg.norm(goal - sol[6:9], ord=2)

    # Now use scipy.optimize.minimize to minimize the function above, using the initial velocity conditions passed in for the third body, and return the initial conditions with the updated third body velocity entries
